...
class FiltroViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        listaParaTableView = filmesDeTerrorClassicos
    }
    
    var filmesDeTerrorClassicos: [Filme] = RelacionamentoAmigos().filmes.filter { filmes in
        filmes.classificacao == .ClassicosDeTerror
    }
    var filmesDeTerrorNovos: [Filme] = RelacionamentoAmigos().filmes.filter { filmes in
        filmes.classificacao == .AtuaisDeTerror
    }
    ...
    @IBAction func escolhapublicoSegmetedControl(_ sender: Any) {
        ...
        case 1:
            listaParaTableView = filmesDeTerrorNovos
            filtroTableView.reloadData()
        default:
            break
        }
...
extension FiltroViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1 {
            if let cell = tableView.dequeueReusableCell(withIdentifier: "filtroCell") as? FiltroTableViewCell {
                cell.configuraCelulaFiltro(poster: filmesDeTerrorNovos[row].posterFilme, nome: filmesDeTerrorNovos[row].nomeFilme)
                return cell
            }
        }
